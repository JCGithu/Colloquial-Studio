<script lang="ts">
  ...
  export let inProgress: EmoteDropParameters;
  ...
  const defaultChart = [
    ...
    [16, inProgress.shape === 1 ? 0.3 : 0.28],
    [20, inProgress.shape === 1 ? 0.375 : 0.35],
    [23, inProgress.shape === 1 ? 0.435 : 0.4],
    [26, inProgress.shape === 1 ? 0.5 : 0.46],
    [32, inProgress.shape === 1 ? 0.6 : 0.57],
    [37, inProgress.shape === 1 ? 0.7 : 0.65],
    [42, inProgress.shape === 1 ? 0.8 : 0.73],
  ...
  onMount(async () => {
    ...
    client.on("chat", (channel, tags, message) => {
      ...
      if (message.startsWith("!emotewipe")) {
        ...
        if (inProgress.modWipe && tags.badges?.moderator) allowed = true;
        if (!allowed) return;
        starWipe();
        return;
      }
  ...
  onTick((delta) => {
    ...
    world.forEachCollider((elt) => {
      ...
      if (inProgress.timeon && item.time < Date.now() - removeTime) deleteThisEmote(item, elt.handle);
    });
    if (emoteMap.size > inProgress.limit) deleteFirstEmote();
  });
